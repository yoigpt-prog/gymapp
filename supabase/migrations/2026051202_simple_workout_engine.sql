-- ============================================================
-- STAGING ONLY — Simple Workout Engine v2.3
-- File    : 20260512_simple_workout_engine.sql
-- Date    : 2026-05-12
-- CHANGE  : Core exercises (abs + obliques) always appended.
--           Final total always EVEN.
--           optional_exercise_ids field added to each day JSON.
--           Cardio days: NO core appended.
--           Exercise selection uses rank + random() jitter for
--           variety on plan regeneration.
-- DO NOT apply to production.
-- ============================================================

DROP FUNCTION IF EXISTS generate_user_workout_plan(uuid);
DROP FUNCTION IF EXISTS resolve_weekly_split(text, int);
DROP FUNCTION IF EXISTS get_exercises_for_day(text, text, text);
...
BEGIN
  ...
  -- ── STEP 1: Read user_preferences ─────────────────────────
  SELECT
    lower(trim(COALESCE(goal, 'build_muscle'))),
    lower(trim(COALESCE(gender, 'male'))),
    lower(trim(COALESCE(training_location, 'gym'))),
  ...
  IF v_goal = 'build_muscle' THEN
    IF    v_days = 3 THEN v_split := ARRAY['push','pull','legs','rest','rest','rest','rest'];
    ELSIF v_days = 4 THEN v_split := ARRAY['upper','lower','upper','lower','rest','rest','rest'];
    ELSIF v_days = 5 THEN v_split := ARRAY['push','pull','legs','upper','fullbody','rest','rest'];
    ELSIF v_days = 6 THEN v_split := ARRAY['push','pull','legs','push','pull','legs','rest'];
  ...
  ELSIF v_goal = 'fat_loss' THEN
    ...
    ELSIF v_days = 4 THEN v_split := ARRAY['upper','lower','rest','cardio','rest','rest','rest'];
    ELSIF v_days = 5 THEN v_split := ARRAY['upper','lower','cardio','fullbody','cardio','rest','rest'];
    ELSIF v_days = 6 THEN v_split := ARRAY['push','pull','legs','cardio','fullbody','cardio','rest'];
    ELSE                  v_split := ARRAY['upper','lower','cardio','fullbody','cardio','cardio','rest'];
    END IF;
  ELSE
...
-- 1. Check abs / obliques data exists
-- SELECT muscle, count(*) FROM exercise_rankings
-- WHERE muscle IN ('abs','obliques') AND gender='male' AND location='gym'
-- GROUP BY muscle;

-- 2. Run plan generation (replace UUID)
-- SELECT generate_user_workout_plan('<staging-uuid>');

-- 3. Inspect day 1 — should show exercises + optional_exercise_ids
-- SELECT
--   schedule_json->'weeks'->'1'->'1'->>'type'           AS day1_type,
--   jsonb_array_length(schedule_json->'weeks'->'1'->'1'->'exercises')             AS total,
--   jsonb_array_length(schedule_json->'weeks'->'1'->'1'->'optional_exercise_ids') AS optional_count,
--   (jsonb_array_length(schedule_json->'weeks'->'1'->'1'->'exercises') % 2 = 0)   AS is_even
-- FROM ai_plans ORDER BY created_at DESC LIMIT 1;
